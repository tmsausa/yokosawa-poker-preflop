  ...
  const generateAllHands = () => {
    ...
    for (let i = 0; i < ranks.length; i++) {
      for (let j = i + 1; j < ranks.length; j++) {
        ...
        hands.push(`${ranks[j]}${ranks[i]}o`); // オフスート
      }
    }
...
​
    return { backgroundColor: colorMap[color], color: ['navy', 'red', 'green', 'gray'].includes(color) ? 'white' : 'black' };
  };
​
  // テーブルを作成
  const createTable = () => {
...
​
export default PokerHandRangeApp;
